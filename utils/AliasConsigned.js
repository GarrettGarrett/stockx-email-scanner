...
export function formatDateMMDDYYY(_date){ //2010-10-11T00:00:00+05:30
...
export async function largeScaleParseAliasConsigned(text, subject, emailDate) {
    ...
    if (line.includes("Name:")){
        rawDetails["title"] = line
    }
    if (line.includes("Amount Made")){
        rawDetails["amountMade"] = line
    }
    ...
    rawDetails["date"] = emailDate.toString()
...
export  function fineParseAliasConsigned(rawDetails) {
    if (
        rawDetails?.subject?.includes("consigned") 
...
export async function lookForMatchConsigned(doc, _fineParseAliasConsignedList){
    await doc.loadInfo();
    let unsoldConsigned = doc.sheetsByTitle["Unsold Consigned"]
    let soldSheet = doc.sheetsByTitle["Sold"]
    const rows = await unsoldConsigned.getRows()
    const rowsSold = await soldSheet.getRows()

    let _fineParseAliasConsignedList_CLEAN_styleId_Size = [] // styleid+size, styleid+size, etc.
    _fineParseAliasConsignedList.forEach(item => {
        ...
        _fineParseAliasConsignedList_CLEAN_styleId_Size.push(_cleanStyleId+_cleanSize)
    })
    ...
    rows.forEach(async (row, index) => {
        ...
        if (_fineParseAliasConsignedList_CLEAN_styleId_Size?.includes(cleanStyleIdSheet+cleanSizeSheet)){
            console.log("Alias Consigned Email StyleId + Size matches Unsold Consigned entry at row: ", index)
            let indexItem = _fineParseAliasConsignedList_CLEAN_styleId_Size?.indexOf(cleanStyleIdSheet+cleanSizeSheet)
...
export async function sendWebhookAliasConsigned(_fineParseArray, webhookUrl) { 
